...
You don't have to setup the project manually but can also use a Maven Archetype (which is like a project template), see [Maven Archetypes](/manual/latest/latest/user-guide/process-applications/maven-archetypes/) for details. The archetype will create a project according to our best practices.
...
You can leave the `META-INF/processes.xml` file empty. In that case, default values are used. See the [Empty Processes.xml](/manual/latest/latest/user-guide/process-applications/the-processes-xml-deployment-descriptor/#empty-processes-xml) section of the [User Guide](/manual/latest/latest/user-guide) for more information
{{< /note >}}
