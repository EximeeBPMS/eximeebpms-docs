...
{{< note title="The recipe targets the Community Edition" class="warning" >}}
`replace-camunda-with-eximeebpms.yml` pins **1.4.0**, a Community Edition version published to Maven Central, and that is what a migrated project lands on. Enterprise Edition artifacts carry an `-ee` suffix and are published to Consdata's own repository, so moving from the migrated Community Edition coordinates to Enterprise ones is a separate step this recipe does not perform.
{{< /note >}}
...
CMMN is removed, so the recipe has **no mapping for `camunda-cmmn-model`** — there is no EximeeBPMS artifact to point it at. A project that uses CMMN cannot be migrated by this recipe; see [CMMN Deprecation & Removal]({{< ref "/update/cmmn-removal.md" >}}).
{{< /note >}}
