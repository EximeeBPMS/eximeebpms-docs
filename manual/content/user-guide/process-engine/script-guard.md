...
Script Guard is an EximeeBPMS feature introduced in 1.3.0 that inspects scripts before execution and blocks — or audits — patterns that could lead to remote code execution, data exfiltration, or other security incidents. It works at the engine level, independently of the scripting language in use.
...
{{< note title="" class="info" >}}
SIEM integration via Business Events will be available from EximeeBPMS 1.4.0. The description below reflects the planned behaviour.
{{< /note >}}
