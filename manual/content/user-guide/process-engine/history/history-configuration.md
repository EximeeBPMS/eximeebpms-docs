...
If you need to customize the amount of history events logged, you can provide a custom implementation {{< javadocref page="org/eximeebpms/bpm/engine/impl/history/producer/HistoryEventProducer.html" text="HistoryEventProducer" >}} and wire it in the process engine configuration.
...
configuration.setHistoryExcludedProcessDefinitionKeys(Set.of("invoice", "credit-check"));

ProcessEngine processEngine = configuration.buildProcessEngine();
```

`setHistoryExcludedProcessDefinitionKeys` is declared on `ProcessEngineConfigurationImpl`, not on the abstract `ProcessEngineConfiguration`, so the configuration has to be typed as the implementation class — a fluent chain starting from `ProcessEngineConfiguration` does not compile. The standard factory methods return a `StandaloneProcessEngineConfiguration`, which extends it.
