...
The ProcessEngines class will scan for all eximeebpms.cfg.xml and activiti.cfg.xml files. For all `eximeebpms.cfg.xml` files, the process engine will be built in the typical way:
...
The **FormService** is an optional service. This means that the EximeeBPMS engine can be used perfectly without it, without sacrificing any functionality. This service introduces the concept of a start form and a task form. A start form is a form that is shown to the user before the process instance is started, while a task form is the form that is displayed when a user wants to complete a task. You can define these forms in the BPMN 2.0 process definition. This service exposes this data in an easy way to work with. But again, this is optional as forms don't need to be embedded in the process definition.
...
[javadocs-query-unlimited-list]: {{< javadocref_url page="org/eximeebpms/bpm/engine/query/Query.html#unlimitedList--" >}}
