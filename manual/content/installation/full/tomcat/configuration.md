...
Make sure the `eximeebpms-identity-ldap-$PLATFORM_VERSION.jar` is present in the
...
Users and Groups in the EximeeBPMS webapplication. In order to activate this, add the following
configuration to the `web.xml` file of EximeeBPMS webapplication
(`eximeebpms-webapp-tomcat-$PLATFORM_VERSION.war/WEB-INF/web.xml`):
...
<filter>
  ...
  <filter-class>org.eximeebpms.bpm.webapp.impl.security.filter.SessionCookieFilter</filter-class>
</filter>
<filter-mapping>
  <filter-name>SessionCookieFilter</filter-name>
